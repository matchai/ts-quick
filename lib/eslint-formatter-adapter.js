 ...
 * @param {string} [cwd] - The directory of the project being tested
 * @returns {Object}
 */
...
function formatDiagnostic(originalDiagnostic) {
  let diagnostic = Object.apply({}, originalDiagnostic);
 ...
 * @param {string} [cwd] - The directory of the project being tested
 */
function splitDiagnosticsByFile(diagnostics, cwd = process.cwd()) {
  /** @type {Object.<string, Object>} */
  const startingObj = {};

  return diagnostics.reduce((acc, cur) => {
    debugger;
    const currentDir = cwd.toLowerCase();
    const filePath = cur.file.fileName.toLowerCase();
